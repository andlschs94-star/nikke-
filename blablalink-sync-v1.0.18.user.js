...
// @version      1.0.18
// @updateURL    https://raw.githubusercontent.com/andlschs94-star/nikke-/main/blablalink-sync-v1.0.18.user.js
// @downloadURL  https://raw.githubusercontent.com/andlschs94-star/nikke-/main/blablalink-sync-v1.0.18.user.js
...
(function capture(){
 ...
 const send=(id,area)=>{try{window.postMessage({type:'NIKKE_GM_CAP',id:id?String(id):'',area:area??null},'*')}catch(_){}};
 const take=(u,obj)=>{
  try{
   const s=String(u||'');
   if(!/blablalink\\.com\\/api\\//i.test(s)) return;
   const id=obj?.data?.info?.intl_openid??obj?.data?.info?.intl_open_id??obj?.data?.intl_openid??obj?.data?.intl_open_id;
   const area=obj?.data?.area_id??obj?.area_id;
   if(id||area) send(id,area);
  }catch(_){}
 };
 try{
  if(!w.__NIKKE_GM_CAPTURE){
   w.__NIKKE_GM_CAPTURE=true;
   const of=w.fetch;
   w.fetch=async function(...a){
    const r=await of.apply(this,a);
    try{const u=String(a[0]?.url||a[0]||''); if(/GetUserGamePlayerInfo|GetUserProfileBasicInfo|GetUserCharacters|GetUserCharacterDetails/.test(u)){const clone=r.clone();clone.json().then(x=>take(u,x)).catch(()=>{});}}catch(_){}
    return r;
   };
   const O=w.XMLHttpRequest;
   const open=O.prototype.open, sendX=O.prototype.send;
   O.prototype.open=function(method,url,...rest){this.__nikkeUrl=url;return open.call(this,method,url,...rest)};
   O.prototype.send=function(body){
    this.addEventListener('load',function(){try{const u=String(this.__nikkeUrl||'');if(/GetUserGamePlayerInfo|GetUserProfileBasicInfo|GetUserCharacters|GetUserCharacterDetails/.test(u)){let x;try{x=JSON.parse(this.responseText)}catch(_){x=null}take(u,x)}}catch(_){}});
    return sendX.call(this,body);
   };
  }
 }catch(e){console.warn('[NIKKE GM] capture install failed',e)}
})(); 

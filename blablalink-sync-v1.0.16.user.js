...
// @version      1.0.16
// @updateURL    https://raw.githubusercontent.com/andlschs94-star/nikke-/main/blablalink-sync-v1.0.16.user.js
// @downloadURL  https://raw.githubusercontent.com/andlschs94-star/nikke-/main/blablalink-sync-v1.0.16.user.js
...
async function sync(token,source){
 ...
 try{
  ...
  const d=p?.data||{};
  const oid=String(d?.info?.intl_openid??d?.intl_openid??'').split('-').pop();
  const area=Number(d?.area_id);
  if(!oid||!area)throw Error('BlablaLink 로그인 정보 확인 실패');
